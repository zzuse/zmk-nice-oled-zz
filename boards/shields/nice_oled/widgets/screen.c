...
// 128x128 I1 needs 2048 bytes + 8 bytes palette = 2056 bytes.
static uint8_t logic_buffer[2064] __attribute__((aligned(4)));

static void draw_text(int x, int y, const char *text) {
    lv_draw_label_dsc_t label_dsc;
    init_label_dsc(&label_dsc, lv_color_black(), &lv_font_unscii_8, LV_TEXT_ALIGN_LEFT);
    
    lv_layer_t layer;
    lv_canvas_init_layer(logic_canvas, &layer);
    // Logical drawing area is 32px wide
...
static void draw_full_canvas(lv_obj_t *widget_obj, const struct status_state *state) {
    ...
    lv_canvas_fill_bg(logic_canvas, lv_color_white(), LV_OPA_COVER);
...
int zmk_widget_screen_init(struct zmk_widget_screen *widget, lv_obj_t *parent) {
    ...
    if (!logic_canvas) {
        ...
        lv_canvas_set_buffer(logic_canvas, (lv_color_t*)logic_buffer, 128, 128, LV_COLOR_FORMAT_I1);
